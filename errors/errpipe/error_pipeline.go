...
type ErrorLoggerConfig struct {
	UserLogLevel   string `env:"USER_ERROR_LOG_LEVEL, default=dont_log" comment:"Log level for the user errors: dont_log, error, warn, info, debug"`
	SystemLogLevel string `env:"SYSTEM_ERROR_LOG_LEVEL, default=error" comment:"Log level for the system errors: dont_log, error, warn, info, debug"`
}
...
func HideInternalError() ErrorProcessor {
	return func(ctx context.Context, err error) error {
		...
		code := err.Error()
		if hint == "" || code == errors.InternalErrorCode {
			resultErr := errsys.New(errors.InternalErrorCode, "Something went wrong")
